...
def finalize_summary_for_export_v17(summary: pd.DataFrame) -> pd.DataFrame:
    ...
    # Mail side - parse the full address to extract components
    full_mail_col = _get_col_ci(df_raw, "matched_mail_full_address")
    if full_mail_col:
        def parse_full_address(full_addr):
            """Parse '123 Main St Apt 2 Dallas TX 75001' -> (street, city, state, zip)"""
            if not full_addr or str(full_addr).strip() == "":
                return "", "", "", ""
            
            addr = str(full_addr).strip()
            # Look for ZIP at the end (5 digits, optionally +4)
...
def _horizontal_bar_chart(month_counts: pd.Series) -> str:
    ...
    return "\n".join(rows), addr)
            if zip_match:
                ...
                addr_no_zip = addr[:zip_match.start()].strip()
            else:
                zip_code = ""
                addr_no_zip = addr
            ...
            if len(parts) >= 2 and len(parts[-1]) == 2 and parts[-1].isalpha():
                state = parts[-1].upper()
                addr_no_state = " ".join(parts[:-1]).strip()
            else:
                state = ""
                addr_no_state = addr_no_zip
            
            # City: take last 1-2 words before state (no digits)
            remaining_parts = addr_no_state.split()
            city_parts = []
            while remaining_parts and not any(c.isdigit() for c in remaining_parts[-1]):
                city_parts.insert(0, remaining_parts.pop())
                if len(city_parts) >= 2:  # Max 2 words for city
                    break
        ...
        # Parse all mail addresses
        parsed = df_raw[full_mail_col].fillna("").apply(parse_full_address)
        mail_street_series = pd.Series([p[0] for p in parsed])
        mail_city_series = pd.Series([p[1] for p in parsed])
        mail_state_series = pd.Series([p[2] for p in parsed])
        mail_zip_series = pd.Series([p[3] for p in parsed])
        mail_cityline = _join_cityline_series(mail_city_series, mail_state_series, mail_zip_series)
    else:
        a1 = df_raw.get(_get_col_ci(df_raw, "address1","mail_address1","mail street","street","addr1","address"), "")
        a2 = df_raw.get(_get_col_ci(df_raw, "address2","mail_address2","unit","addr2","line2"), "")
